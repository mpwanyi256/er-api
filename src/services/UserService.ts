...
import KeystoreRepo from './KeystoreService';
...
async function findPrivateProfileById(
  ...
  .select(
    '+_id +email +password +roles +profile +status',
  )
  .populate({
    path: 'roles',
    match: { status: true },
  ...
  .populate({
    path: 'profile',
    match: { status: true },
    select: { firstName: 1, lastName: 1, mobileNumber: 1, profilePictureURL: 1 },
  })
  .lean<User>()
  .exec();
}
...
async function findById(id: Types.ObjectId): Promise<User | null> {
  return UserModel.findOne({ _id: id, status: true })
    .select('+email +password +roles +profile +status')
    .populate({
      path: 'roles',
      match: { status: true },
    })
...
async function findByEmail(email: string): Promise<User | null> {
  return UserModel.findOne({ email: email })
    .select(
      '+email +password +roles +profile +status +emailVerified  +createdAt',
    )
    .populate({
      path: 'roles',
      match: { status: true },
